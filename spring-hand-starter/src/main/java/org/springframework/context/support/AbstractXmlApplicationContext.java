...
public abstract class AbstractXmlApplicationContext extends AbstractRefreshableConfigApplicationContext{
    ...
    protected void loadBeanDefinitions(DefaultListableBeanFactory beanFactory) throws IOException {
        ...
        //   DefaultListableBeanFactory的追踪如下，档案馆会先被XmlBeanDefinitionReader存储，然后放入到reader的上下文中(最终创建上下文的方法是dooadBeanDefinitions)，最后将解析器解析
        //的结果放入到从上下文中拿到的DefaultListableBeanFactory中，以此来完成档案馆的加载也就是刷新
        XmlBeanDefinitionReader beanDefinitionReader = new XmlBeanDefinitionReader(beanFactory);

        // Configure the bean definition reader with this context's
        // resource loading environment.
        // TODO beanDefinitionReader.setEnvironment(getEnvironment());
        // 该方法在抽象类中，每一个Reader都需要有一个loader来加载资源
        beanDefinitionReader.setResourceLoader(this);
        // 这是xml读取器特有的属性，我们暂时定义一下，因为我们不做具体的实现，所以里面什么都没有,但是话又说回来，这是jdk的类，那就无所谓了，话又说回来，实现类是Spring的那就又有事了
